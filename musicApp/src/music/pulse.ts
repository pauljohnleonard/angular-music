...
export class Pulse extends Savable implements Thing {
    ...
    removeClient(client: Ticker): void {
        ...
        for (; index < this.clients.length; index++) {
    ...
    getTimeOfBeat(beat: number): number {
        const dBeat: number = beat - this.beat

        const t: number = this.time + dBeat * 60 / this.bpm
    ...
    */
    getBeatNow(): any {
        const dT = this.getTime() - this.time
        const beat = this.beat + dT * this.bpm / 60
        return beat
    }
    ...
    loadSnap(pulseSnap: any) {
        ...
        const sig = pulseSnap.child("sig").val()
